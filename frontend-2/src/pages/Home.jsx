...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Attempt silent SSO — will succeed if the user logged in on Frontend 1 (or any app
    // using the same Keycloak realm) because Keycloak's session cookie is accessible.
    auth.signinSilent()
      .then(() => {
        navigate('/dashboard', { replace: true });
      })
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        ...
        <p style={styles.description}>
          This is <strong>{THEME.appName}</strong> running on <code>localhost:3002</code>.
          If you logged in on{' '}
          <a href="http://localhost:3001" style={{ color: THEME.primary }}>App Alpha (port 3001)</a>{' '}
          you should have been automatically signed in — no password needed.
        </p>
        <button
...
const styles = {
  ...
  card: {
    ...
    maxWidth: 480,
    width: '100%',
    boxShadow: '0 4px 24px rgba(13,115,71,0.1)',
    textAlign: 'center',
  },
  badge: {
